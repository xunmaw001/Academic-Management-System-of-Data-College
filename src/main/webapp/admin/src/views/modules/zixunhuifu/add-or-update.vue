...
export default {
  ...
  methods: {
	...
	this.ruleForm.huifuneirong = this.ruleForm.huifuneirong.replace(reg,'../../../ssm8r63x/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zixunhuifu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zixunhuifuCrossAddOrUpdateFlag = false;
						 this.parent.search();
