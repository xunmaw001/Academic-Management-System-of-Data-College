<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.xiangguanfujian" label="相关附件" prop="xiangguanfujian">
          <file-upload
          tip="点击上传相关附件"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.xiangguanfujian?ruleForm.xiangguanfujian:''"
          @change="xiangguanfujianUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.xiangguanfujian" label="相关附件" prop="xiangguanfujian">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.xiangguanfujian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="要求时间" prop="yaoqiushijian">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.yaoqiushijian" 
                type="date"
                :readonly="ro.yaoqiushijian"
                placeholder="要求时间">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.yaoqiushijian" label="要求时间" prop="yaoqiushijian">
              <el-input v-model="ruleForm.yaoqiushijian" 
                placeholder="要求时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="作业内容" prop="zuoyeneirong">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.zuoyeneirong" label="作业内容" prop="zuoyeneirong">
                    <span v-html="ruleForm.zuoyeneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.buzhishijian = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zuoyemingcheng = true;
          ...
          if(o=='xiangguanfujian'){
            this.ruleForm.xiangguanfujian = obj[o];
	    this.ro.xiangguanfujian = true;
            continue;
          }
          if(o=='jiaoshigonghao'){
	    ...
	    this.ro.buzhishijian = true;
          ...
          if(o=='yaoqiushijian'){
            this.ruleForm.yaoqiushijian = obj[o];
	    this.ro.yaoqiushijian = true;
            continue;
          }
          if(o=='zuoyeneirong'){
	...
	this.ruleForm.zuoyeneirong = this.ruleForm.zuoyeneirong.replace(reg,'../../../ssm8r63x/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.xiangguanfujian!=null) {
		this.ruleForm.xiangguanfujian = this.ruleForm.xiangguanfujian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kechengzuoye/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kechengzuoyeCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    xiangguanfujianUploadChange(fileUrls) {
	this.ruleForm.xiangguanfujian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
